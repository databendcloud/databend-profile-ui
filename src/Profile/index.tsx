...
import { useDeviceContextProvider } from '../Context/deviceContext';
...
const Profile: FC<IProps> = ({
  ...
  const { isPhone } = useDeviceContextProvider();
  ...
  // const { run: runGetSqlProfile, loading: initLoading } = useDatabendRequest(
  //   getSqlProfile,
  //   {
  ...
  //     await runGetSqlProfile(orgSlug, queryId);
  //   }
  //   if (orgSlug) {
  //     fetchData();
  //   }
  ...
  useUpdateEffect(() => {
    if (outValue) {
      try {
        dealData(JSON.parse(outValue));
      } catch (error) {
        message.destroy();
        message.warning('数据有误');
      }
    }
  }, [countShow]);

  useMount(() => {
    dealData(JSON.parse(DEMO));
  });
  ...
  const handleResize = () => {
    if (profileRef) {
      setGraphSize(
        isPhone
          ? profileRef?.offsetWidth
          : profileRef?.offsetWidth - offsetWidth,
      );
    }
            ...
            >
              <div className="profile-common-styles-few g-box-center-between">
                ...
                {!isTotalOverview && (
                  <Tooltip
                    title={
                      overviewInfo?.metrics ? undefined : '此节点未生成 Metrics'
                    }
                  >
                    <Button
